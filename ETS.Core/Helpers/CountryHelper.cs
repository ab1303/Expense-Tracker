...
namespace UGC.Core.Helpers
{
    public class CountryHelper
    {
        ...
        static CountryHelper()
        {
            ...
            // Alternate method. But it doesn't have ISO Country Code.
            //foreach (var ci in CultureInfo.GetCultures(types: CultureTypes.AllCultures))
            //{
            //    RegionInfo ri;
            //    try
            //    {
            ...
            //    if (countries.Any(x => x.DisplayName == ri.EnglishName) || ri.EnglishName.Equals("World", StringComparison.CurrentCulture))
            //        continue;

            //    // Create new country dictionary entry.                
            //    countries.Add(new Country
            //    {
            //        DisplayName = ri.EnglishName,
            ...
            Countries = countries.OrderBy(x => x.DisplayName).ToList();
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(countryName)) throw new ArgumentNullException();
            return Countries.Single(x => x.DisplayName == countryName).CountryCode;
        }

        public static string GetCountryNameByCountryCode(string countryCode)
        {
            if(string.IsNullOrWhiteSpace(countryCode)) return null;
            return Countries.Single(x => x.CountryCode == countryCode).DisplayName;
        }

        public static string GetCountryTwoLetterRegionName(string countryName)
        {
            if (string.IsNullOrWhiteSpace(countryName)) throw new ArgumentNullException();
            return Countries.Single(x => x.DisplayName == countryName).TwoLetterRegionName;
        }
    }
    ...
    public class Country
    {
        ...
        public string TwoLetterRegionName { get; internal set; }
        public string ThreeLetterRegionName { get; internal set; }
        public string CountryCode { get; internal set; }

        internal Country(string displayName, string twoLetterRegionName, string threeLetterRegionName, string countryCode)
        {
            DisplayName = displayName;
            TwoLetterRegionName = twoLetterRegionName;
            ThreeLetterRegionName = threeLetterRegionName;
